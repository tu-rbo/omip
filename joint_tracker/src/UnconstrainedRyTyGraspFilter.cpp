...
UnconstrainedRyTyGraspFilter::UnconstrainedRyTyGraspFilter() :
    JointFilter(),
    _sys_PDF(NULL),
    _sys_MODEL(NULL),
    _meas_PDF(NULL),
...
void UnconstrainedRyTyGraspFilter::initialize()
{
...
void UnconstrainedRyTyGraspFilter::_initializeSystemModel()
{
...
void UnconstrainedRyTyGraspFilter::_initializeMeasurementModel()
{
...
void UnconstrainedRyTyGraspFilter::_initializeEKF()
{
    ...
    // We initialize with the identity matrix as Jacobian except the 2nd and 5th columns
    for(int i =0; i<6; i++)
    {
        if(i != 4 && i != 1)
        {
...
UnconstrainedRyTyGraspFilter::UnconstrainedRyTyGraspFilter(const UnconstrainedRyTyGraspFilter &rev_joint) :
...
void UnconstrainedRyTyGraspFilter::predictState(double time_interval_ns)
{
...
void UnconstrainedRyTyGraspFilter::predictMeasurement()
{
    _delta_eef_wrt_cf_ec = (_rrb_pose_in_sf.exp(1e-12)*(_rrb_previous_pose_in_sf.exp(1e-12).inverse())).log(1e-12);

    _delta_eef_wrt_previous_eef_ec =  _rrb_previous_pose_in_sf.exp(1e-12).inverse().adjoint()*_delta_eef_wrt_cf_ec;

    // There are no changes in the relative pose between the end effector and the interacted rigid body
    // EXCEPT in the rotation in y and translation along y: the motion of the end-effector is not transmitted to the object, so their
    // relative pose changes by that amount
    this->_change_in_relative_pose_predicted_in_rrbf = Eigen::Twistd(0., -_delta_eef_wrt_previous_eef_ec.ry(), 0., 0., -_delta_eef_wrt_previous_eef_ec.vy(), 0.);
    ...
    for(int i=0; i<6; i++)
    {
        for(int i=0; i<6; i++)
        {
            for(int j=0; j<6; j++)
            {
                ...
                {
                    if(i == j)
                        this->_change_in_relative_pose_cov_predicted_in_rrbf(i,j) = 0.5;
                    else
                        this->_change_in_relative_pose_cov_predicted_in_rrbf(i,j) = 0.1;
                }else
                {
                    if(i == j)
                        this->_change_in_relative_pose_cov_predicted_in_rrbf(i,j) = 0.00005;
                    else
                        this->_change_in_relative_pose_cov_predicted_in_rrbf(i,j) = 0.00001;
                }
            }
        }
    }
...
void UnconstrainedRyTyGraspFilter::correctState()
{
}

void UnconstrainedRyTyGraspFilter::estimateUnnormalizedModelProbability()
{
    this->_unnormalized_model_probability = _model_prior_probability*_measurements_likelihood;
}

void UnconstrainedRyTyGraspFilter::estimateMeasurementHistoryLikelihood()
{
    if(!_externally_set_likelihood)
    {
        ...
        for (size_t sample_idx = 0; sample_idx < amount_samples; sample_idx++)
        {
            ...
            Eigen::Displacementd rb2_last_delta_relative_displ_ryty_hyp = Eigen::Twistd(0.,
                                                                                         this->_changes_in_relative_pose_in_rrbf.at(this->_changes_in_relative_pose_in_rrbf.size()-sample_idx-1).ry(), //Allow any rotation in y
                                                                                         0.,
                                                                                         0,
                                                                                         this->_changes_in_relative_pose_in_rrbf.at(this->_changes_in_relative_pose_in_rrbf.size()-sample_idx-1).vy(), //Allow any translation in y
                                                                                         0).exp(1e-12);
            Eigen::Vector3d rb2_last_delta_relative_translation_ryty_hyp = rb2_last_delta_relative_displ_ryty_hyp.getTranslation();
            Eigen::Quaterniond rb2_last_delta_relative_rotation_ryty_hyp = Eigen::Quaterniond(rb2_last_delta_relative_displ_ryty_hyp.qw(),
                                                                                               rb2_last_delta_relative_displ_ryty_hyp.qx(),
                                                                                               rb2_last_delta_relative_displ_ryty_hyp.qy(),
                                                                                               rb2_last_delta_relative_displ_ryty_hyp.qz());


            // Distance proposed by park and okamura in "Kinematic calibration using the product of exponentials formula"
            double translation_error = (rb2_last_delta_relative_translation - rb2_last_delta_relative_translation_ryty_hyp).norm();

            Eigen::Quaterniond rotation_error = rb2_last_delta_relative_rotation.inverse() * rb2_last_delta_relative_rotation_ryty_hyp;
...
JointFilterType UnconstrainedRyTyGraspFilter::getJointFilterType() const
{
    return UNCONSTRAINED_RY_TY_GRASP_JOINT;
}

std::string UnconstrainedRyTyGraspFilter::getJointFilterTypeStr() const
{
    return std::string("UnconstrainedRyTyGraspFilter");
}

std::vector<visualization_msgs::Marker> UnconstrainedRyTyGraspFilter::getJointMarkersInRRBFrame() const
{
    ...
    transmitted_translation_marker.color.r = 0.5f;
    transmitted_translation_marker.color.g = 0.f;
    transmitted_translation_marker.color.b = 0.5f;
    ...
    transmitted_rotation_marker.color.r = 0.5f;
    transmitted_rotation_marker.color.g = 0.f;
    transmitted_rotation_marker.color.b = 0.5f;
