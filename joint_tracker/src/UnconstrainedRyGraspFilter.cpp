...
UnconstrainedRyGraspFilter::UnconstrainedRyGraspFilter() :
    JointFilter(),
    _sys_PDF(NULL),
    _sys_MODEL(NULL),
    _meas_PDF(NULL),
    ...
    _pp_weight(1)
{
    ...
    this->_model_prior_probability = 1.0/3.955;
}

void UnconstrainedRyGraspFilter::initialize()
{
...
void UnconstrainedRyGraspFilter::_initializeSystemModel()
{
...
void UnconstrainedRyGraspFilter::_initializeMeasurementModel()
{
...
void UnconstrainedRyGraspFilter::_initializeEKF()
{
    ...
    // We initialize with the identity matrix as Jacobian except the 5th column
    for(int i =0; i<6; i++)
    {
        if(i != 4)
        {
...
UnconstrainedRyGraspFilter::UnconstrainedRyGraspFilter(const UnconstrainedRyGraspFilter &rev_joint) :
...
void UnconstrainedRyGraspFilter::predictState(double time_interval_ns)
{
...
void UnconstrainedRyGraspFilter::predictMeasurement()
{
    ...
    // EXCEPT in the rotation in y: the motion of the end-effector is not transmitted to the object, so their
    // relative pose changes by that amount
    this->_change_in_relative_pose_predicted_in_rrbf = Eigen::Twistd(0., -_delta_eef_wrt_previous_eef_ec.ry(), 0., 0., 0., 0.);
...
void UnconstrainedRyGraspFilter::correctState()
{
}

void UnconstrainedRyGraspFilter::estimateMeasurementHistoryLikelihood()
{
    if(!_externally_set_likelihood)
    {
        ...
        for (size_t sample_idx = 0; sample_idx < amount_samples; sample_idx++)
        {
            ...
            Eigen::Displacementd rb2_last_delta_relative_displ_ry_hyp = Eigen::Twistd(0.,
                                                                                         ...
                                                                                         0.,
                                                                                         0.).exp(1e-12);
            Eigen::Vector3d rb2_last_delta_relative_translation_ry_hyp = rb2_last_delta_relative_displ_ry_hyp.getTranslation();
            Eigen::Quaterniond rb2_last_delta_relative_rotation_ry_hyp = Eigen::Quaterniond(rb2_last_delta_relative_displ_ry_hyp.qw(),
                                                                                               rb2_last_delta_relative_displ_ry_hyp.qx(),
                                                                                               rb2_last_delta_relative_displ_ry_hyp.qy(),
                                                                                               rb2_last_delta_relative_displ_ry_hyp.qz());

            // Distance proposed by park and okamura in "Kinematic calibration using the product of exponentials formula"
            double translation_error = (rb2_last_delta_relative_translation - rb2_last_delta_relative_translation_ry_hyp).norm();

            Eigen::Quaterniond rotation_error = rb2_last_delta_relative_rotation.inverse() * rb2_last_delta_relative_rotation_ry_hyp;
...
void UnconstrainedRyGraspFilter::estimateUnnormalizedModelProbability()
{
    this->_unnormalized_model_probability = _model_prior_probability*_measurements_likelihood;
}

JointFilterType UnconstrainedRyGraspFilter::getJointFilterType() const
{
    return UNCONSTRAINED_RY_GRASP_JOINT;
}

std::string UnconstrainedRyGraspFilter::getJointFilterTypeStr() const
{
    return std::string("UnconstrainedRyGraspFilter");
}

std::vector<visualization_msgs::Marker> UnconstrainedRyGraspFilter::getJointMarkersInRRBFrame() const
{
    ...
    transmitted_translation_marker.color.r = 1.f;
    transmitted_translation_marker.color.g = 1.f;
    transmitted_translation_marker.color.b = 0.f;
    ...
    transmitted_rotation_marker.color.r = 1.f;
    transmitted_rotation_marker.color.g = 1.f;
    transmitted_rotation_marker.color.b = 0.f;
